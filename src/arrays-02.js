...
function makeWordTitleCase(word) {
  ...
  const isUpperNow = firstLetter.toUpperCase();
  const restLower = word.slice(1);
  const isLowerNow = restLower.toLowerCase();
  const finalWord = isUpperNow + isLowerNow;
  return finalWord;
}
...
function makeSentenceTitleCase(sentence) {
  ...
  for (let a = 0; a < array.length; a++) {
    array[a] = array[a].charAt(0).toUpperCase() + array[a].slice(1);
  }
